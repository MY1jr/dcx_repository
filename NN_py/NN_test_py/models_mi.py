...
import numpy as np
from attentions import MultiHeadAttention, CBAM, MaskCSAttention, FrameDiffAttention, BADSAttention
#######################################################################################
class MI_GTNet(nn.Module):
    """
    多输入: [batch, time_step, channel, height, width]
    ...
    def __init__(self, in_channels=3, out_channels=3, height=64, width=64, time_step=20):
        ...
        # 空间注意力
        self.SpatialAttention = MaskCSAttention(embed_dim=16)
        # 共享的特征提取层
    ...
    def forward(self, x_orig, x_argu=None):  # x: [batch, time_step, channel, height, width]
        x = x_orig
        batch_size = x.size(0)
        time_step = x.size(1)
        ...
        if self.training: # mask只在训练时使用
            mask = x_argu
            mask = self.input_layer(mask)
        ...
        for t in range(self.time_step):
            ...
            if self.training: # mask只在训练时使用
                mask_t = mask[:,t,:,:,:]  # [batch, 32, h, w]
                x_t = self.SpatialAttention(x_orig=x_t, x_argu=mask_t)
...
class ConvGroup(nn.Module):
    ...
    def __init__(self, input_dim):
        ...
        # Stage 5: [batch, 512, h/4, w/4] -> [batch, 256, h/4, w/4]
        ...
        # Stage 6: [batch, 256, h/4, w/4] -> [batch, 128, h/8, w/8]
