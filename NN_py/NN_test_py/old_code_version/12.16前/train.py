...
###############################################################################
    ...
    # 每5轮保存一次中间权重
    SAVE_WEIGHT_EPOCH_NUM = 5
    # 每20轮更新一次后处理，累计，覆盖之前的
    SAVE_DATA_EPOCH_NUM = 20
    # best_Loss初始化
    best_loss = float('inf')
    # 记录训练和测试损失
    train_losses = []
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % SAVE_DATA_EPOCH_NUM == 0:
            ...
            ## 保存误差数据到csv
            save_errordata(speed_mae_list, angle_mae_list, speed_mre_list, angle_mre_list, save_path=errordata_save_path)
