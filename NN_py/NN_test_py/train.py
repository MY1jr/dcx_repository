...
def img_transform(img):
    ...
    # img = cv2.resize(img, (100, 100)) # convlstm
    img = img / 255.0  # 图像归一化
...
if __name__ == "__main__":
    ...
    # pth_load_path = 'G:/NN_py/NN_test_py/best.pth'
    ...
    losscurve_save_path = current_dir / 'loss_curves.png'
...
###############################################################################
    ...
    train_dataloader = DataLoader(
        ...
        shuffle=True,  # 随机打乱训练数据 ##### 待测试shuffle ######
        num_workers=4,  # 多进程加载
...
###############################################################################
    ...
    try:    
        checkpoint = torch.load(pth_load_path, weights_only=True)
...
###############################################################################
    ...
    for epoch in range(start_epoch, num_epochs):  # 从start_epoch开始训练
        ...
        # 保存角度和速度损失
        angle_loss = angle_loss_meter.avg
        angle_losses.append(angle_loss)
        speed_loss = speed_loss_meter.avg
        ...
        print(f'测试集角度损失: {angle_loss:.6f}')
        print(f'测试集速度损失: {speed_loss:.6f}')
        ...
        if (epoch + 1) % SAVE_DATA_EPOCH_NUM == 0:
            ...
            ## 保存训练损失数据到csv    
            save_detail_lossdata(train_losses, test_losses, angle_losses, speed_losses, save_path=lossdata_save_path, start_epoch=start_epoch+1)
            ## 保存误差数据到csv
