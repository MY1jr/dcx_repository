...
class SingleOutputWrapper:
    ...
    def __getitem__(self, idx):
        ...
        # data是一个元组(img1, img2, data)，我们只返回img1
        return data[0], label  # 只返回第一个图像和标签
class DoubleOutputWrapper:
    def __init__(self, dataset):
        self.dataset = dataset
    def __len__(self):
        return len(self.dataset)
...
if __name__ == "__main__":
    ...
    time_step = 20
    # 保存路径
    pth_save_dir = 'G:/NN_py/NN_test_py/pth'
    pth_load_path = 'G:/NN_py/NN_test_py/best_mi.pth'
    losscurve_save_path = 'G:/NN_py/NN_test_py/loss_curves.png'
    lossdata_save_path = 'G:/NN_py/NN_test_py/loss_data.csv'
    errordata_save_path = 'G:/NN_py/NN_test_py/error_data.csv'
###############################################################################
    # # 设置随机种子以确保可重复性
    ...
    source_dataset = mi_dcxDataset( 
        label_txt="G:/NN_py/dcx_mini_merge/dcx_mini_merge_final.txt", ### 三输入数据集 ###
        time_step=time_step,
    ...
    # 随机划分
    train_dataset_raw, test_dataset_raw = torch.utils.data.random_split(
        source_dataset, 
        [train_size, test_size]
    )
    
    # 将训练集包装为双输出格式
    train_dataset = DoubleOutputWrapper(train_dataset_raw)
    # 将测试集包装为单输出格式
    test_dataset = SingleOutputWrapper(test_dataset_raw)
...
###############################################################################
    ...
    # 学习率调度器
    ## 超参数 ##
    num_warmup_epochs = int(num_epochs * 0.1)  # warmup轮数10% = 10轮
    warmup_factor = 0.01    # warmup起始学习率因子
    ...
    try:    
        ...
        try:
            # 恢复优化器状态
            # optimizer.load_state_dict(checkpoint['optimizer_state_dict']) # 换优化器要注释掉这句
            # 恢复学习率调度器状态
...
###############################################################################
    ...
    for epoch in range(start_epoch, num_epochs):  # 从start_epoch开始训练
        ...
        total_train_loss = 0
        # 单次训练
        train_pbar = tqdm(train_dataloader, desc='training',position=0)
        for batch_idx, (data, target) in enumerate(train_pbar):
            
            detected_seq, cluster_seq = data
            detected_seq = detected_seq.to(device)
            cluster_seq = cluster_seq.to(device)
            target = target.to(device)
            
            optimizer.zero_grad()
            output = model(detected_seq, cluster_seq)
        ...
        test_pbar = tqdm(test_dataloader, desc='testing',position=0)
        with torch.no_grad():
