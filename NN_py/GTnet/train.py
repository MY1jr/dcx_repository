...
from models.gtnet import GTNet
...
if __name__ == "__main__":
    ...
    num_epochs = 50
    time_step = 20
    angle_learning_rate = 0.0001 # 0.0001
    speed_learning_rate = 0.0001 # 0.0001
    # 断点重训
...
###############################################################################
    ...
    source_dataset = dcxDataset(
        label_txt=str(project_root / 'dcx' / 'dcx.txt'), 
        # label_txt=str(project_root / 'ssstest' / 'ssstest.txt'),
...
###############################################################################
    ...
    for epoch in range(start_epoch, num_epochs):  # 从start_epoch开始训练
        ...
        for batch_idx, (img_seq, target) in enumerate(train_pbar):
            # 将所有输入数据移到设备上
            ...
            # 更新损失计量器
            train_loss_meter.update(total_loss.item(), img_seq.size(0))
            train_angle_loss_meter.update(angle_loss.item(), img_seq.size(0))
            train_speed_loss_meter.update(speed_loss.item(), img_seq.size(0))
        ...
        with torch.no_grad():
            for batch_idx, (img_seq, target) in enumerate(test_pbar):
                # 将所有输入数据移到设备上
                img_seq = img_seq.to(device)
                target = target.to(device)
                ...
                # 更新误差计量器
                speed_mae_meter.update(speed_mae.item(), img_seq.size(0))
                angle_mae_meter.update(angle_mae.item(), img_seq.size(0))
                speed_mre_meter.update(speed_mre.item(), img_seq.size(0))
                angle_mre_meter.update(angle_mre.item(), img_seq.size(0))
                
                # 更新计量器
                test_loss_meter.update(total_loss.item(), img_seq.size(0))
                angle_loss_meter.update(angle_loss.item(), img_seq.size(0))
                speed_loss_meter.update(speed_loss.item(), img_seq.size(0))
